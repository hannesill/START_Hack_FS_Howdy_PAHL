        ...
        <nav class="flex flex-1 flex-col">
          <ul role="list" class="flex flex-1 flex-col gap-y-7">
            <li>
              <ul role="list" class="-mx-2 space-y-1">
                <li v-for="item in navigation" :key="item.name">
                  <router-link :to="item.href" class="text-gray-400 hover:text-white group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold" active-class="bg-gray-800 text-white">
                    <component :is="item.icon" class="h-6 w-6 shrink-0" aria-hidden="true" />
                    {{ item.name }}
                  </router-link>
                </li>
...
const navigation = [
  ...
  { name: 'Founders', href: '/founders', icon: UsersIcon, current: false },
  { name: 'Startups', href: '/startups', icon: RocketLaunchIcon, current: false },
]
